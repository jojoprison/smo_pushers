...
def gen_service_time_teller2():  # function to generate service time for teller 1 using inverse transform
    return -np.log(1 - (np.random.uniform(low=0.0, high=1.0))) * 1.5


def gen_service_time_teller1():  # function to generate service time for teller 1 using inverse transform
    return -np.log(1 - (np.random.uniform(low=0.0, high=1.0))) * 1.2


def gen_int_arr():  # function to generate arrival times using inverse transform
...
class Simulation:
    def __init__(self):
        ...
        self.t_departure2 = float('inf')  # cx time from server 2
    ...
    def arrival(self):
        ...
        if self.num_in_q == 0:  # schedule next departure or arrival depending on state of servers
            if self.state_T1 == 1 and self.state_T2 == 1:
                ...
                if np.random.choice([0, 1]) == 1:
                    self.state_T1 = 1
                    self.dep1 = gen_service_time_teller1()
                    ...
                    self.state_T2 = 1
                    self.dep2 = gen_service_time_teller2()
        ...
        elif 4 > self.num_in_q >= 1:  # if queue length is less than 4 generate next arrival and make customer join queue
            self.num_in_q += 1
            self.number_in_queue += 1
            self.t_arrival = self.clock + gen_int_arr()
